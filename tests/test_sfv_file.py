...
sfv_file_compare = [
    hoarder.hash_file.FileEntry(
        pathlib.Path("empty_file"), 0, False, b"\x00\x00\x00\x00", hoarder.hash_file.Algo.CRC32
    ),
    hoarder.hash_file.FileEntry( # 902F433D 902F433D
        pathlib.Path("english text.txt"), 637, False, b"\x90\x2f\x43\x3d", hoarder.hash_file.Algo.CRC32
    ),
    hoarder.hash_file.FileEntry(
        pathlib.Path("Stück.txt"), 964, False, b"\xb3\xa6\x5a\xf4", hoarder.hash_file.Algo.CRC32
    ),
    hoarder.hash_file.FileEntry(
        pathlib.Path("trailing whitespace .txt"), 729, False, b"\x47\x5f\x91\x8b", hoarder.hash_file.Algo.CRC32
    ),
    hoarder.hash_file.FileEntry(
        pathlib.Path("日本語テキスト.txt"), 5070, False, b"\x80\xed\x75\x5f", hoarder.hash_file.Algo.CRC32
    ),
]

def test_sfv_files():
    sfv_file_path = (test_file_path / ".." / "test_files" / "sfv" / "rhash_output.sfv").resolve()
    ...
    sfv_file_lowercase_path = (test_file_path / ".." / "test_files" / "sfv" / "rhash_output_lowercase.sfv").resolve()
