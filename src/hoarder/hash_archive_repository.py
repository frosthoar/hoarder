...
class HashArchiveRepository:
    ...
    def save(self, archive: HashArchive) -> None:
        ...
        with Sqlite3FK(self._db_path) as con:
            ...
            if archive.files:
                fe_rows = list(self._build_fileentry_rows(archive.files, archive_path=archive.path))
                _ = cur.executemany(
                    """
                    INSERT INTO file_entries (path, size, is_dir, hash_value, algo, archive_id)
                    SELECT :path AS path, :size AS size, :is_dir AS is_dir, :hash_value AS hash_value, :algo AS algo, archive_id FROM
                    hash_archives WHERE archive_path = :archive_path
                    """,
                    fe_rows
                )
            con.commit()
    ...
    def load(self, path: pathlib.Path | str) -> HashArchive | None:
        ...
        with Sqlite3FK(self._db_path) as con:
            ...
            arc_row = cast(
                None | sqlite3.Row,
                cur.execute(
                    "SELECT * FROM hash_archives WHERE path = ?;", (str(path),)
                ).fetchone(),
            )
    ...
    def _ensure_schema(self) -> None:
        with Sqlite3FK(self._db_path) as con:
            cur = con.cursor()
            _ = cur.executescript(
                self._CREATE_HASH_ARCHIVES + "\n" + self._CREATE_FILE_ENTRIES
            )
    ...
    def _fill_archive(row: sqlite3.Row) -> HashArchive:
        ...
        if archive_type == "HashNameArchive":
            ...
            arch = RarArchive(
                ...
                scheme=RarScheme(cast(str, row["rar_scheme"]))
