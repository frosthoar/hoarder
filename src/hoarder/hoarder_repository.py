...
class HoarderRepository:
    ...
    def __init__(
        self, db_path: str | Path, allowed_storage_paths: collections.abc.Iterable[Path | str]
    ) -> None:
    ...
    def load_hash_archive(
        self, storage_path: Path | str, path: PurePath | str
    ) -> HashArchive:
    ...
    def load_real_file(self, storage_path: Path | str, path: PurePath | str) -> RealFile:
        normalized_storage_path = self._check_storage_path_allowed(storage_path)
        with Sqlite3FK(self.db_path) as con:
            self._ensure_storage_path(con, normalized_storage_path)
    ...
    def _check_storage_path_allowed(self, storage_path: Path | str) -> Path:
        normalized = Path(storage_path).resolve()
